...
async def cancel_commands(message: types.Message,
                          state: FSMContext):
...
async def fsm_start(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        await message.reply("Admin, send me a photo")
...
async def load_title(message: types.Message,
    ...
    await FSMADMIN.next()
    await message.reply("Send me description of photo")
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(cancel_commands, state='*', commands=['cancel'])
    dp.message_handler(cancel_commands, Text(equals='cancel', ignore_case=False))